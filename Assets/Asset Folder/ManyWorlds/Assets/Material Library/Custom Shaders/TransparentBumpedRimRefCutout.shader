...
/////// Normals:
                ...
                float3 normalLocal = UnpackNormal(tex2D(_NormalMap,TRANSFORM_TEX(node_810.rg, _NormalMap))).rgb;
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
                float3 viewReflectDirection = reflect( -viewDirection, normalDirection );
                float4 node_2 = tex2D(_Base,TRANSFORM_TEX(node_810.rg, _Base));
                clip((lerp( node_2.a, tex2D(_CustomAlphaMap,TRANSFORM_TEX(node_810.rg, _CustomAlphaMap)).a, _CustomAlpha )*_AlphaIntensity) - 0.5);
                #ifndef LIGHTMAP_OFF
...
/////// Diffuse:
                ...
                #endif
////// Emissive:
                float4 node_35 = tex2D(_SpecularRGBGlossA,TRANSFORM_TEX(node_810.rg, _SpecularRGBGlossA));
...
/////// Normals:
                ...
                float3 normalLocal = UnpackNormal(tex2D(_NormalMap,TRANSFORM_TEX(node_811.rg, _NormalMap))).rgb;
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
                float3 viewReflectDirection = reflect( -viewDirection, normalDirection );
                float4 node_2 = tex2D(_Base,TRANSFORM_TEX(node_811.rg, _Base));
                clip((lerp( node_2.a, tex2D(_CustomAlphaMap,TRANSFORM_TEX(node_811.rg, _CustomAlphaMap)).a, _CustomAlpha )*_AlphaIntensity) - 0.5);
...
///////// Gloss:
                float4 node_35 = tex2D(_SpecularRGBGlossA,TRANSFORM_TEX(node_811.rg, _SpecularRGBGlossA));
                float gloss = (node_35.a*_GlossLevel);
                float specPow = exp2( gloss * 10.0+1.0);
////// Specular:
                NdotL = max(0.0, NdotL);
...
/// Final Color:
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float4 node_2 = tex2D(_Base,TRANSFORM_TEX(node_812.rg, _Base));
                clip((lerp( node_2.a, tex2D(_CustomAlphaMap,TRANSFORM_TEX(node_812.rg, _CustomAlphaMap)).a, _CustomAlpha )*_AlphaIntensity) - 0.5);
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float4 node_2 = tex2D(_Base,TRANSFORM_TEX(node_813.rg, _Base));
                clip((lerp( node_2.a, tex2D(_CustomAlphaMap,TRANSFORM_TEX(node_813.rg, _CustomAlphaMap)).a, _CustomAlpha )*_AlphaIntensity) - 0.5);
